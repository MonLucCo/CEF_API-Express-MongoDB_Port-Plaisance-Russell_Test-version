...
Ce document décrit l’ensemble des tests techniques du projet et couvre les niveaux 1 à 4 :

- tests unitaires
- tests d’intégration
- tests E2E simulés
- tests de pré‑déploiement / déploiement / post‑déploiement

Ces tests garantissent la stabilité technique du code et la qualité de la version déployée.

---

## Préliminaire - Préambule

Ce document est conservé dans sa rédaction initiale menée lors des travaux de développement.
...
2) **Tests opérationnels Client** (`npm run tests`)
   ...
   Ces tests sont développés en Phase 7 ( détail dans `tests-client.md`).
